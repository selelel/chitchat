...
import { UserSignUpContext } from '../_context/signup-context'
import {
    user_credential_schema,
    user_credential_type,
} from '@/lib/schemas/signin.form.dto'
...
export const SignUpUserCredentialForm = () => {
    const context = useContext(UserSignUpContext)
    const {
        register,
        handleSubmit,
        formState: { errors },
    } = useForm<user_credential_type>({
        resolver: yupResolver(user_credential_schema),
    })
    const [checkIfEmailExists, { isLoading }] =
    ...
    return (
        <Form
            submit={handleSubmit(validateUserEmailAndPassword)}
            className="space-y-2"
        >
            ...
            >
                ...
                <p
                    className={`font-semibold text-custom-grey ${poppins.className}`}
                >
                    Google
                </p>
            </Button>
            ...
            <div className="flex flex-col w-full">
                <h4
                    className={`font-semibold text-xs text-custom-1 ${poppins.className}`}
                >
                    Email Address
                </h4>
                <Form.Input
