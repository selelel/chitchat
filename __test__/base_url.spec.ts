...
test.describe('LoginPage', () => {
    ...
    test('should have metadata and elements', async ({ page }) => {
        for (const id of Object.values(pw_id.login))
            await expect(page.getByTestId(id)).toBeVisible()
    })

    test('should log-in user', async ({ page }) => {
        await page
            .getByTestId(pw_id.login.eml_inp)
            .fill(mock_values.login_1.valid.eml)
        await page
            .getByTestId(pw_id.login.pass_inp)
            .fill(mock_values.login_1.valid.pass)
